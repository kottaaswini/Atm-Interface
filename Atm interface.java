...
public class Project {
	...
	private void delete(long acc_num) throws IOException {
		...
		while (line != null) {
			if (line.equals(Long.toString(acc_num))) {
				...
				// StringBuilder converts a given data to a string and then appends or inserts
				// the characters of that string to the string builder
				StringBuilder sb = new StringBuilder();
				while ((line = br.readLine()) != null) {
					if (line.equals(Long.toString(acc_num))) {
						for (int counter = 0; counter < 11; counter++) // Skipping the lines of the account details
																		// which is to be deleted
							br.readLine();
					} else {
						// Appending all the lines to string builder except the account number to be
						// deleted
						sb.append(line);
						sb.append("\n");
	...
	private void deleteCustomer(long acc_num) throws IOException {
		...
		while (line != null) {
			if (line.equals(Long.toString(acc_num))) {
				...
				// If acc number and PIN matches then deleting that account using delete method
	...
	protected class Banker {
		...
		String setDate() {
			...
			if (!(checkDate(Integer.parseInt(accDate[0]), Integer.parseInt(accDate[1]),
					Integer.parseInt(accDate[2])))) {
		...
		private void deleteAdmin() throws IOException {
			...
			if (admin_id.equals("admin") && admin_pass.equals("admin@123")) {
				...
				if (!adminDatabase.exists()) {
					// If admin database is empty it will write a default admin id and password in
					// the file
					...
					if (line.equals(admin_id)) {
						...
						if (line.equals(admin_pass)) {
							BufferedReader br = new BufferedReader(
									new FileReader(new File("./Admin/adminDatabase.txt")));
							StringBuilder sb = new StringBuilder();
							while ((line = br.readLine()) != null) {
								if (line.equals(admin_id)) {
									br.readLine();
		...
		private void insert(String acc_type, String name, String gender, String nationality, String kyc, String doa,
				String dob, long acc_num, short pin, long mob_num, long acc_bal) throws IOException {
		...
		// Method function that displays the sub menu of functions available for admin
		// and asks for choice
	...
	protected class ATM {
		...
		private void withdraw(long acc_num) throws IOException {
				...
				if (line.equals(Long.toString(acc_num))) {
					...
					if (line.equals(Short.toString(pin))) {
						...
						if (amount > currentBalance) {
							...
							System.out.println("\nSum of Rs. " + amount
									+ " withdrawn successfully. Your updated balance is Rs. " + currentBalance);
						}
			...
			if (flag == 1) {
				...
				// Appending all the lines to StringBuilder and changing previous account
				// balance with current account balance
				line = br.readLine();
				sb.append(line);
				sb.append("\n");
				...
				if (line.equals(Long.toString(acc_num))) {
					...
					if (line.equals(Short.toString(pin))) {
						...
						if (amount > 25000) {
							...
							currentBalance += amount;
							System.out.println("\nSum of Rs. " + amount
									+ " deposited successfully. Your updated balance is Rs. " + currentBalance);
						}
		...
		private void changePIN(long acc_num) throws IOException {
			...
			while (line != null) {
				if (line.equals(Long.toString(acc_num))) {
					...
					if (line.equals(Short.toString(pin))) {
						// Asking for the input of new PIN and storing it in val and validating and
			...
			if (flag == 1) {
				...
				// Appending all the lines to StringBuilder but in place of PIN we will append
				// the new PIN set by user.
				line = br.readLine();
				sb.append(line);
				sb.append("\n");
		...
		private void showBalance(long acc_num) throws IOException {
			...
			while (line != null) {
				if (line.equals(Long.toString(acc_num))) {
					...
					// If account number an PIN matches with the data in database, then current
		...
		protected void function() throws IOException {
			...
			while (true) {
				...
				// Enhanced switch case will execute/call the methods according to the choice
				// made by customer
				switch (option) {
...
class Main {
	public static void main(String[] args) throws IOException {
		...
		while (true) {
			...
			// According the choice of user calling methods from project class using the
				...
				case 1: // If user chooses to operate as a banker
					while (true) {
						...
						if (!adminDatabase.exists()) {
							...
							adminDatabase.createNewFile();
							BufferedWriter writeAdminDatabase = new BufferedWriter(
									new FileWriter("./Admin/adminDatabase.txt"));
							writeAdminDatabase.write("admin\nadmin@123\n*\n");
						}

						BufferedReader readAdminDatabase = new BufferedReader(
								new FileReader("./Admin/adminDatabase.txt"));
						String line = readAdminDatabase.readLine();
						int flag = 0;
